...
- [x] MelSpectrogram feature extraction (FFT-based, 40 mel bands, 98 frames)
- [x] hello_vasu.tflite model bundled in assets/wakeword/ (26,188 bytes, shape [1, 98, 40] -> [1, 1])
- [x] Real TFLite model inference validated (noise < 0.2, wake speech >= 0.70)
...
### Phase 5: Gemini Live Native Audio & Natural Female Voice (TTS)
- [x] Gemini Live Native Audio integrated via GeminiLiveSession (OkHttp WebSocket)
- [x] NativeAudioPlayer with hardware AudioTrack (24kHz, 16-bit Mono PCM, hardware draining)
- [x] Prebuilt female voice: "Kore"
- [x] testKoreVoice() executes the real Gemini Live native audio pipeline
- [x] Android Fallback TTS with female voice profile for offline emergency use
